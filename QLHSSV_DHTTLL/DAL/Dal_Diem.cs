...
namespace DAL
{
    public class Dal_Diem : connect
    {
        ...
        {
            da = new SqlDataAdapter("Select MASV, MAMH, HOCKY, DIEMCC, DIEMTX, DIEMGK, DIEMTHI, ( DIEMCC + DIEMTX + DIEMGK*3 + DIEMTHI*5 )/10 as DiemTongKet From DIEM", dbConn);
            dt = new DataTable();
            da.Fill(dt);
            dbConn.Close();
            return dt;
        ...
        {
            dbConn.Open();
            string cmd = "UPDATE DIEM SET MAMH=N'" + pKT.MaMonHoc + "',HOCKY='" + pKT.HocKy + "', DIEMCC='" + pKT.DiemCC + "', DIEMTX= '" + pKT.DiemTX + "', DIEMGK= '" + pKT.DiemGK + "', DIEMTHI= '" + pKT.DiemCK + "'  WHERE MASV='" + pKT.MaSinhVien + "'";
            SqlCommand sqlCmd = new SqlCommand(cmd, dbConn);
            sqlCmd.ExecuteNonQuery();
            dbConn.Close();
            return true;
        ...
        {
            dbConn.Open();
            string cmd = "DELETE FROM DIEM WHERE MASV='" + maSV + "'";
            SqlCommand sqlCmd = new SqlCommand(cmd, dbConn);
            sqlCmd.ExecuteNonQuery();
            dbConn.Close();
            return true;
